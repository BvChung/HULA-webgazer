...
var isWebGazerReady = false;

// Calibration variables
var isCalibrating = false;
var calibrationStartTimestamp = 0;
var calibrationFrameReq = null;
...
async function getPrediction(regModelIndex) {
	...
	if (predictions.length === 0 || predictions[0] === null) return null;
...
async function loop() {
	...
	loopFrameReq = requestAnimationFrame(loop);
}

/**
 * Calibration runs every available animation frame
 */
async function calibrationLoop() {
	...
	calibrationFrameReq = requestAnimationFrame(calibrationLoop);
}
...
async function init(stream) {
	...
	const videoPreviewSetup = new Promise((resolve) => {
...
/**
 * Initalize webgazer with a video stream
 * @param {Function} onFail - The callback function to run if the webcam access fails
 * @returns {webgazer} this
 */
webgazer.initialize = async function (onFail) {
	if (
	...
	onFail =
		onFail ||
		function () {
			console.error("Unable to access webcam");
		};
...
webgazer.isReady = function () {
	return isWebGazerReady && videoStream && videoStream.getTracks().length > 0;
};
...
webgazer.stopVideo = function () {
	...
	if (videoContainerElement && faceOverlay) {
		videoContainerElement.removeChild(faceOverlay);
	}

	// Removes the box around the face
	if (videoContainerElement && faceFeedbackBox) {
 ...
 * @param {"TFFacemesh"} name - The name of the tracking module to use
 * @return {webgazer} this
 */
 ...
 * @param {"ridge" | "weightedRidge" | "threadedRidge"} name - The name of the regression module to use
 * @return {webgazer} this
 */
