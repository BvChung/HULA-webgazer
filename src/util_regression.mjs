...
util_regression.InitRegression = function () {
	...
	// Calibration training to determine the X, Y weights for the regression model
	this.XWeights = [];
	this.YWeights = [];

	this.calibrationScreenXClicksArray = [];
	this.calibrationScreenYClicksArray = [];
	this.calibrationEyeFeaturesClicks = [];

	//sets to one second worth of cursor trail
	this.calibrationTrailTime = 1000;
	this.calibrationTrailDataWindow = this.trailTime / params.moveTickSize;
	this.calibrationScreenXTrailArray = [];
	this.calibrationScreenYTrailArray = [];
	this.calibrationEyeFeaturesTrail = [];
	this.calibrationTrailTimes = [];

	this.calibrationDataClicks = [];
	this.calibrationDataTrail = [];
...
util_regression.addCalibrationData = function (eyes, screenPos, type) {
	if (!eyes) {
		return;
	}

	if (type === "click") {
		this.calibrationScreenXClicksArray.push([screenPos[0]]);
		this.calibrationScreenYClicksArray.push([screenPos[1]]);
		this.calibrationEyeFeaturesClicks.push(util.getEyeFeats(eyes));
		this.calibrationDataClicks.push({
			eyes: eyes,
			screenPos: screenPos,
			type: type,
		});
	} else if (type === "move") {
		this.calibrationScreenXTrailArray.push([screenPos[0]]);
		this.calibrationScreenYTrailArray.push([screenPos[1]]);
		this.calibrationEyeFeaturesTrail.push(util.getEyeFeats(eyes));
		this.calibrationTrailTimes.push(performance.now());
		this.calibrationDataTrail.push({
			eyes: eyes,
			screenPos: screenPos,
			type: type,
		});
...
util_regression.clearCalibrationData = function () {
	this.calibrationScreenXClicksArray = [];
	this.calibrationScreenYClicksArray = [];
	this.calibrationEyeFeaturesClicks = [];
	this.calibrationScreenXTrailArray = [];
...
util_regression.getCalibrationData = function () {
	return {
		screenXClicksArray: this.calibrationScreenXClicksArray,
		screenYClicksArray: this.calibrationScreenYClicksArray,
		eyeFeaturesClicks: this.calibrationEyeFeaturesClicks,
		screenXTrailArray: this.calibrationScreenXTrailArray,
		screenYTrailArray: this.calibrationScreenYTrailArray,
		eyeFeaturesTrail: this.calibrationEyeFeaturesTrail,
		trailTimes: this.calibrationTrailTimes,
		dataClicks: this.calibrationDataClicks,
		dataTrail: this.calibrationDataTrail,
	};
};
